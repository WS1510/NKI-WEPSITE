...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Delegated: buttons with data-target scroll to that selector
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', async function(e) {
            ...
            if (isValid) {
                ...
                // client-side send: first try server /api/quote
                const sendClient = async (payload) => {
                    // prepare a JSON body; attachments may include dataUrl if present
                    try{
                        // Determine server endpoint. If page is not served from the same port as the API (3000),
                        // use an absolute URL to the local server so Live Server (5501) pages still reach the API.
                        const serverEndpoint = (window.NKI_API_ENDPOINT) || ((location.port && location.port !== '3000') ? 'http://127.0.0.1:3000/api/quote' : '/api/quote');
                        const res = await fetch(serverEndpoint, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify(payload)
                        });
                        if (res.ok) return { ok: true, via: 'server', data: await res.json() };
                        // if server returns non-ok, fallthrough to other methods
                    }catch(e){
                        // network error or server down, fallthrough to fallbacks
                    }

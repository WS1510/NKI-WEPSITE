...
async function createTransporter() {
    if (process.env.SMTP_HOST && process.env.SMTP_USER) {
        ...
        // try to verify SMTP connection; if it fails (DNS or auth), fall back to test account
        try {
            await t.verify();
            ...
            // continue to create ethereal below
        }
    }
...
// 파일 기반 로그 저장 (간단한 JSON 배열). 동시성 단순 처리(append): 읽어서 push 후 쓰기.
...
// Update a log entry (mark handled / add note)
